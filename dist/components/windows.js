...
$(document).ready(function () {
  ...
  $(".open-menu").on("click", function () {
    if (launchpad.hasClass("shown start")) closeLaunchpad();else openLaunchpad();
  });
  ...
  $(".mac-terminal, .text-edit, .email, .calc, .notes, .browser, .dialogue").position({
    my: "center center-36.5",
    // Subtract menubar height (3rem = 28.8px when font-size is 9.6px = 60%) from vertical center
    at: "center",
    collision: "fit",
    of: "#main-content"
  });
  ...
  $(document).mouseup(function (e) {
    ...
    if (content.is(e.target) || nav.is(e.target)) closeLaunchpad();
  });
  ...
  function bringToFront(element, allElements) {
    ...
    var maxZIndex = Math.max.apply(Math, _toConsumableArray($(allElements).map(function () {
